...
export default function BusinessCartDrawer({
  ...
  const checkoutMp = async () => {
    ...
    try {
      ...
      if (data.checkoutUrl) {
        ...
        alert(data.error || 'Checkout no disponible. Usa WhatsApp.');
      }
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-[250] flex justify-end bg-black/50 backdrop-blur-sm print:hidden">
      <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={item.productId} className="flex gap-3 p-3 rounded-2xl border border-slate-100">
                {item.imageUrl && (
