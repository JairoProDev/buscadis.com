...
export default function ConductorRegistroPage() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    const res = await fetch('/api/envios/rider', { credentials: 'include' });
    ...
    if (data.rider) {
      ...
      if (data.rider.estado === 'aprobado') {
        router.replace('/envios/conductor');
      }
    }
    setDocs(data.docs || []);
    setLoading(false);
  ...
  const uploadDoc = async (tipo: MotoDocType, file: File) => {
    ...
    try {
      ...
      fd.append('tipo', tipo);
      fd.append('file', file);
      const res = await fetch('/api/envios/rider/docs', {
        method: 'POST',
        credentials: 'include',
        body: fd,
      });
    ...
    return (
      <div className="min-h-screen bg-[var(--bg-primary)]">
        ...
        <main className="mx-auto max-w-lg px-4 py-10 text-center">
          ...
          <Link href="/login?redirect=/envios/conductor/registro" className="text-[var(--brand-blue)]">
            Login
          </Link>
        </main>
      </div>
